    ...
    def step(self, mcs):
        ...
        for cell in self.cellListByType(1, 2):
            ...
            if mcs < 9:
                ...
                # cellDict=self.getDictionaryAttribute(cell) print >>fileHandle,mcs,",",cellDict["comX"][i],",",
                # cellDict["comY"][i],",",cellDict["comX"][i+1],",",cellDict["comY"][i+1],",",diffX,",",diffY,",",
                # sumX,",",sumY,",",avgX,",",avgY fileHandle.close()
            ...
            cellDict = self.getDictionaryAttribute(cell)
            print(mcs, ",", cell.xCOM, ",", cell.yCOM, ",", sumX, ",", sumY, ",", avgX, ",", avgY, ",",
                  cellDict['vecX'], ",", cellDict['vecY'], file=fileHandle)
            fileHandle.close()
...
class MMPSecretionSteppable(SteppableBasePy):
    ...
    def step(self, mcs):
        ...
        print(mcs, ",", totalMMPProduced, ",", totalMMPPresent, file=fileHandle)
        fileHandle.close()
...
class ECMDegradationSteppable(SteppableBasePy):
    ...
    def step(self, mcs):
        ...
        print(mcs, ",", totalFibersDeleted, file=fileHandle)
        fileHandle.close()
